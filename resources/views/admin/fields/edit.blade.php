...
@section('content')
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-heading font-bold text-slate-800 dark:text-white">Cập Nhật Sân Bóng</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Chỉnh sửa thông tin sân bóng: {{ $field->name }}</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $field->name) }}" required class="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-xl px-4 py-3 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-primary/50 shadow-inner">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="field_type_id" required class="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-xl px-4 py-3 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-primary/50 shadow-inner">
                                    ...
                                    @foreach($fieldTypes as $type)
                                        <option value="{{ $type->id }}" {{ old('field_type_id', $field->field_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }} (Sức chứa: {{ $type->capacity }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="base_price" value="{{ old('base_price', intval($field->base_price)) }}" required min="0" step="1000" class="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-xl px-4 py-3 pr-12 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-primary/50 shadow-inner">
                        ...
                        <div class="grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-700 pt-4">
                            <div>
                                ...
                                <select name="status" required class="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-xl px-4 py-3 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-primary/50 shadow-inner">
                                    <option value="available" {{ old('status', $field->status) == 'available' ? 'selected' : '' }}>Trống (Sẵn sàng)</option>
                                    <option value="booked" {{ old('status', $field->status) == 'booked' ? 'selected' : '' }}>Đã Đặt</option>
                                    <option value="in_use" {{ old('status', $field->status) == 'in_use' ? 'selected' : '' }}>Đang Đá</option>
                                    <option value="maintenance" {{ old('status', $field->status) == 'maintenance' ? 'selected' : '' }}>Đang Bảo Trì</option>
                                </select>
                            ...
                            <div class="flex items-center mt-8">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $field->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" class="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-xl px-4 py-3 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-primary/50 shadow-inner">{{ old('description', $field->description) }}</textarea>
                            @error('description') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <!-- Image Upload -->
                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Cập Nhật Ảnh Sân</label>
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 dark:border-slate-600 border-dashed rounded-xl hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group cursor-pointer">
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-slate-600 dark:text-slate-400 justify-center">
                                        ...
                                        <p class="pl-1">để ghi đè ảnh cũ</p>
                                    </div>
